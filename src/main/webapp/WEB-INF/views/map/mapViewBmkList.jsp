...
<title>Insert title here</title>
	<style type="text/css">
		...
		#bmkListByCtgry {width:350px; display:block; border-right:1px solid #E2E2E2; height:100%; overflow:auto;}
...
<body>
	<div id="totalContainer">
		...
		<div id="bmkListByCtgry">			
			
			<h3>전체 카테고리</h3>
			<h4 id="goBack" onclick='fn_goBackPg()'>&lt;</h4>
			<h4 id="bmkListRemove">X</h4>
			<br>
			<ol>
				<table>
					<tbody>
						<c:forEach items="${bmkList}" var="bmkList" varStatus="status">
							<tr>
								<td>
								<a href="#" onclick="panTo(${bmkList.x}, ${bmkList.y})">
									<span id="bmkItem"></span>
									<div>${bmkList.place_name}</div>
									<div>${bmkList.road_address_name}</div>
									<div>${bmkList.address_name}</div>
									<div>${bmkList.phone}</div>
									<button type="button" onclick='fn_delBookmark()'>북마크 삭제</button>
								</a>
								</td>								
							</tr>
						</c:forEach>
		...
		<div id="mapContent">
			...
			<button type="button" id="categoryList" display="none" >목록 보기</button>
		</div>	<!-- mapContent -->
...
<script>
	...
	bmkListRemove.addEventListener('click', function(){
		var slideDiv = document.querySelector('#bmkListByCtgry');		
